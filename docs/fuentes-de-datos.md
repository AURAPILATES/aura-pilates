...
7. **Alerta proactiva por email (jun 2026).** Cuando un cron falla (tras agotar los 3 reintentos), `logSyncRun` (`lib/syncRuns.ts`) llama a `sendSyncFailureAlert` (`lib/alertEmail.ts`), que envía un email vía la API REST de **Resend** con el error exacto y la hora. No hace falta esperar a entrar al dashboard para enterarse de un fallo.
   - **Destinatario:** `juliamuliterno@gmail.com` por defecto. Para cambiarlo, añadir la variable de entorno `ALERT_EMAIL` en Vercel con la dirección deseada.
   - **Remitente:** `onboarding@resend.dev` (dominio de prueba de Resend, gratis hasta 3.000 emails/mes — no requiere verificar dominio propio).
   - **Configuración necesaria:** variable de entorno `RESEND_API_KEY` en Vercel (Settings → Environment Variables → Production), generada en resend.com → API Keys. Sin esta variable, el sistema sigue funcionando igual pero solo deja constancia en consola, no envía el email.
   - **Si falla el envío del email en sí** (p.ej. Resend caído), no se reintenta ni bloquea el cron — solo se registra en consola; el fallo original sigue quedando en `sync_runs` y visible en el panel.

**Pendiente / fuera de alcance:** esto no cubre una caída de Momence de varias horas que coincida con ambos intentos del día (poco probable, pero posible) — en ese caso el snapshot de ese día se perdería igual, aunque sí se recibiría el email de aviso de cada intento fallido. Tampoco hay alerta por WhatsApp (se descartó por requerir verificación de negocio en Meta/Twilio y tener coste por mensaje).
