...
function NewContactForm({ categories, onCreated, onCancel }: {
  ...
  return (
    <div className="px-3 py-3 border-b border-navy/[0.05] bg-navy/[0.015] space-y-2">
      <div className="flex gap-2">
        <input
          type="text" placeholder="Concepto (ej. TRANSFER. EN DIV.)"
          value={concept} onChange={(e) => setConcept(e.target.value)}
          className="flex-1 px-2 py-1 text-sm border border-navy/15 rounded-md focus:outline-none focus:border-primary/40"
        />
        <input
          type="text" placeholder="Contacto / Más datos (ej. Stripe Technology Eu)"
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 text-xs text-navy/45 shrink-0">
          IVA <input type="text" inputMode="decimal" value={ivaRate} onChange={(e) => setIvaRate(e.target.value)}
            className="w-14 px-1.5 py-1 text-xs text-right border border-navy/15 rounded-md focus:outline-none focus:border-primary/40" />%
        </div>
        <div className="flex items-center gap-1 text-xs text-navy/45 shrink-0">
          Ret. <input type="text" inputMode="decimal" value={retencionRate} onChange={(e) => setRetencionRate(e.target.value)}
            className="w-14 px-1.5 py-1 text-xs text-right border border-navy/15 rounded-md focus:outline-none focus:border-primary/40" />%
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button
          onClick={onCancel}
          className="px-3 py-1.5 text-xs text-navy/55 border border-navy/15 rounded-md hover:bg-navy/[0.03] transition-colors"
        >
          Cancelar
        </button>
        <button
          onClick={handleSave}
          disabled={saving}
          className="px-3 py-1.5 text-xs font-semibold bg-navy text-white rounded-md hover:bg-navy/85 transition-colors disabled:opacity-50"
        >
