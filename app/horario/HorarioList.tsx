    ...
    <div className="space-y-8">
      {days.map(({ dateKey, events: dayEvents }) => {
        ...
        return (
          <div key={dateKey}>
            {/* Day header */}
            <div className="flex items-baseline justify-between gap-4 pb-3 border-b border-navy/[0.09]">
              ...
              <div className="flex items-start gap-8 shrink-0">
                ...
                <DayStat label="ALUMNOS"   value={`${dayStudents}/${dayTotal}`} />
                <DayStat label="LIBRES"    value={String(dayFree)} valueClass={dayFree === 0 ? "text-navy/30" : "text-navy"} />
...
function DayStat({
  label, value, valueClass = "text-navy",
}: {
