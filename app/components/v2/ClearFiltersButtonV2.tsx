...
export default function ClearFiltersButtonV2({ onClick, className = "" }: { onClick: () => void; className?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`shrink-0 flex items-center gap-1 text-[12.5px] font-medium rounded-[8px] px-2.5 py-[7px] text-primary hover:text-primary/75 transition-colors whitespace-nowrap ${className}`}
    >
