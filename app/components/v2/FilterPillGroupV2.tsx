...
export default function FilterPillGroupV2<T extends string>({ options, active, onChange, className = "" }: Props<T>) {
  return (
    <div className={`inline-flex gap-0.5 bg-[#f6f6f7] rounded-[10px] p-[3px] ${className}`}>
      {options.map(({ key, label, count, countTone }) => (
        <button
          key={key}
          type="button"
          onClick={() => onChange(key)}
          className={`flex items-center gap-1.5 px-3.5 py-[7px] rounded-[8px] text-[13px] font-medium transition-colors whitespace-nowrap ${
