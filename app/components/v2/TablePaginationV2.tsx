...
export default function TablePaginationV2({ page, totalItems, pageSize, onPageChange }: Props) {
  ...
  const prevBtn = (
    <button
      ...
      disabled={safePage === 0}
      className="w-7 h-7 flex items-center justify-center rounded-[8px] text-[#a1a1aa] hover:text-[#18181b] hover:bg-[#18181b]/[0.04] disabled:opacity-30 disabled:cursor-not-allowed transition-colors shrink-0"
    >
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-1.5 py-3.5">
      <p className="text-[12.5px] text-[#a1a1aa] whitespace-nowrap shrink-0">
      ...
      {totalPages > 1 && (
        <>
          ...
          <div className="sm:hidden flex items-center gap-1 shrink-0">
            {prevBtn}
            <span className="text-xs font-medium text-[#18181b] px-1 whitespace-nowrap">{safePage + 1} / {totalPages}</span>
            {nextBtn}
          </div>
